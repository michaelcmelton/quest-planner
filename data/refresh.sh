...
# Get JSON
curl https://oldschool.runescape.wiki/api.php\?action\=ask\&format\=json\&query\=%5B%5BQuest%20JSON%3A%3A%2B%5D%5D%7C%3FQuest%20JSON%7Climit%3D10000\&formatversion\=2 > ./data/quest_json.json

# prevent the printing of the directory with command substitution
sed -iE 's/\* /*/g' ./data/quest_json.json
echo $(cat ./data/quest_json.json | jq '.query.results') > ./data/quest_json.json

# Replace HTML Enitities with proper characters
FILE_CONTENTS=$(cat ./data/quest_json.json | perl -CS -MHTML::Entities -ne 'print decode_entities($_)') 
...
echo "$FILE_CONTENTS" > ./data/updated.json

echo $(cat ./data/updated.json | jq 'with_entries(.value |= .printouts."Quest JSON")') > ./data/final_json.json
rm ./data/quest_json.jsonE